...
def test_parameterized_sql_not_flagged(tmp_path):
    _write(tmp_path, "q2.py",
           "cursor.execute('SELECT * FROM users WHERE id = $1', uid)\n")
    findings = _run(tmp_path)
...
def test_dangerously_set_inner_html_flagged(tmp_path):
    _write(tmp_path, "Foo.tsx",
           "const x = <div dangerouslySetInnerHTML={{__html: untrusted}} />;\n")
    findings = _run(tmp_path)
